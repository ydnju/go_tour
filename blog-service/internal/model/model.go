...
// Blank Import: 引入mysql的引用，调用mysql的init方法做一定的初始化工作
import (
	"fmt"
	"time"
	...
	// Import mysql plugin with implicit import, so init() method of
	// mysql plugin package could be executed.
...
// 结构体字段加反引号表示tag
// tag由反引号包围，由一对或者几对的键值对组成，空格分隔
// 获取tag分三个步骤:获取字段field，获取tag，解析键值对
